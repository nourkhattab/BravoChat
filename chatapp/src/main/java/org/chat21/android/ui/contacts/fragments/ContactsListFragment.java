...
public class ContactsListFragment extends Fragment {
    ...
    private CopyOnWriteArrayList<IChatUser> getRestrecredContacts() {
        ...
        CopyOnWriteArrayList<IChatUser> restrictedContacts = new CopyOnWriteArrayList<>(); //= contactsSynchronizer.getContacts();
                ...
                for (IChatUser contact : contactsSynchronizer.getContacts()) {
                    for (String id : iDs){
                       if(contact.getId().equals(id))
                           restrictedContacts.add(contact);
                    }
                }
            }
        if(restrictedContacts.size() > 0)
            return restrictedContacts;
        else
            return contactsSynchronizer.getContacts();
    }
