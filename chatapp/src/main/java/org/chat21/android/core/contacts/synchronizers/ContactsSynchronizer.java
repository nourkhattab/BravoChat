...
import android.demo.AppContext;
...
//        });
        ...
        if (contactsChildEventListener == null) {
            ...
            contactsChildEventListener = contactsNode.addChildEventListener(new ChildEventListener() { //orderByChild("firstname")
                @Override
                        ...
                        protected Void doInBackground(final DataSnapshot... snapshots) {
                            new Thread(new Runnable() {
                                public void run() {
                                    ...
                                    try {
                                        Log.d(DEBUG_CONTACTS_SYNC, "Count ContactsSynchronizer.connect.onChildAdded.contact");
...
//                            Log.v(DEBUG_CONTACTS_SYNC, "ContactsSynchronizer.connect.onChildChanged");
                    ...
                    try {
                        String token = ChatManager.getInstance().getAuthorizationToken();
                        if(token != null) {
...
//                    }.execute(dataSnapshot);
                ...
                public void onChildRemoved(DataSnapshot dataSnapshot) {
                    ...
                    try {
                        ...
                        List<String> iDs = ChatManager.getInstance().getRestrictedUsersIds();
                        if(iDs.size() > 0) {
